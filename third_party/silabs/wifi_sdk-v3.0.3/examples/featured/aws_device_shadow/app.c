...
static void simulate_room_temperature(int16_t *p_room_temperature, bool *p_window_open);
...
sl_status_t start_aws_device_shadow()
{
  ...
  window_actuator.cb         = NULL;

  temperature_handler.pKey       = "Temperature";
  temperature_handler.pData      = &temperature;
  temperature_handler.dataLength = sizeof(int16_t);
  temperature_handler.type       = SHADOW_JSON_INT16;
...
static void simulate_room_temperature(int16_t *p_room_temperature, bool *p_window_open)
{
  ...
  *p_window_open = false;
  ...
  if (*p_room_temperature >= (int16_t)(ROOM_TEMPERATURE_UPPER_LIMIT + ROOM_TEMPERATURE_LOWER_LIMIT) / 2) {
    *p_window_open = true;
  }
}
