...
typedef enum { RSI_NONE, RSI_TKIP, RSI_CCMP } rsi_encryption_mode_t;
...
typedef struct {
  ...
  uint8_t reserved[4];
  si91x_scan_info_t scan_info[RSI_AP_SCANNED_MAX];
} si91x_rsp_scan_t;
...
} si91x_req_cert_valid_t;

// Set certificate command request structure
typedef struct {
  si91x_cert_info_t cert_info;                   // certificate information structure
  uint8_t certificate[SI91X_MAX_CERT_SEND_SIZE]; // certificate
} si91x_req_set_certificate_t;
...
} si91x_disassociation_request_t;
#pragma pack()
...
} si91x_network_params_response_t;
...
typedef struct {
  ...
  si91x_station_info_t sta_info[SI91X_MAX_STATIONS];
} si91x_client_info_response;
...
typedef struct si91x_req_rx_stats_s {
  // 0 - start , 1 -stop
  uint8_t start[2];
...
} si91x_req_rx_stats_t;

//! wlan per stats structure
typedef struct si91x_per_stats_rsp_s {
  uint8_t tx_pkts[2];
...
typedef struct {
  ...
  si91x_fd_set_t rsi_read_fds;

  si91x_fd_set_t rsi_write_fds;

  si91x_time_value rsi_select_timeval;
...
typedef struct si91x_req_multicast {
  // IP version
  uint8_t ip_version[2];
...
} __attribute__((__packed__)) si91x_http_client_put_start_t;

typedef struct {
  //! Current chunk length
  uint16_t current_length;
} __attribute__((__packed__)) si91x_http_client_put_data_request_t;
...
typedef struct {
  ...
  union {
    ...
    si91x_http_client_put_start_t http_client_put_start;

    //! HTTP PUT PACKET command structure
    si91x_http_client_put_data_request_t http_client_put_data_req;
...
} __attribute__((__packed__)) si91x_http_client_put_request_t;
...
} si91x_http_client_post_data_request_t;
...
} si91x_http_client_put_pkt_rsp_t;
...
typedef struct {
  ...
  uint32_t offset;
  uint32_t data_len;
} si91x_http_put_pkt_server_rsp_t;
#endif
...
} __attribute__((__packed__)) si91x_request_ta2m4_t;
#endif
#endif
