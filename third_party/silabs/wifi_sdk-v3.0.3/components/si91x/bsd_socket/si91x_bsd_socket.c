...
#define TCP_RX_WINDIW_DIVISION_FACTOR 10

#define SI91X_SOCKET_FEAT_SSL          BIT(0)
#define SI91X_HIGH_PERFORMANCE_SOCKET  BIT(7)

// TLS version
#define SI91X_TLS_V_1_0 BIT(2)
#define SI91X_TLS_V_1_2 BIT(3)
#define SI91X_TLS_V_1_1 BIT(4)
#ifdef CHIP_917
#define SI91X_TLS_V_1_3 BIT(8)
#endif
...
typedef enum {
  ...
  UDP_UNCONNECTED_READY, // (UDP ONLY STATE) Socket attains this state when sento() or recvfrom() has been executed successfully prior connect.
...
static sl_si91x_socket_config_t socket_config = {
  ...
  TCP_RX_WINDIW_DIVISION_FACTOR    // TCP RX window division factor
};
...
int16_t si91x_get_socket_mss(int32_t socketIndex)
{
  ...
  if ((si91x_socket->type == SOCK_STREAM) && si91x_socket->state == CONNECTED) {
    if (si91x_socket->ssl_bitmap & SI91X_SOCKET_FEAT_SSL) {
...
static void socket_disconnect_event_handler(sl_si91x_event_t event, sl_status_t status, void *data, void *user_data)
{
 ...
 * @return sl_status 			SL_STATUS_OK in case of success or appropriate sl_stattus error value.
 */
static sl_status_t si91x_bsd_send_driver_command(rsi_wlan_cmd_request_t command,
                                                 const void *data,
                                                 uint32_t data_length,
                                                 si91x_queue_type_t queue,
                                                 si91x_queue_type_t response_queue,
...
static int32_t create_and_send_socket_request(int32_t socketIdIndex, int32_t type, int32_t *backlog)
{
  ...
  if (si91x_bsd_socket->local_address.sin6_family == AF_INET6) {
    socket_create_request.ip_version = SL_IPV6_ADDRESS_LENGTH;
  ...
  if (si91x_bsd_socket->ssl_bitmap & SI91X_SOCKET_FEAT_SSL) {
...
int socket(int family, int type, int protocol)
{
  ...
  if (!is_configured) {
    ...
    sl_si91x_register_callback(SL_SI91X_SOCKET_REMOTE_TERMINATED_EVENT, socket_disconnect_event_handler, NULL);
    is_configured = true;
  }
...
int accept(int socket_id, struct sockaddr *addr, socklen_t *addr_len)
{
  ...
  si91x_client_socket->remote_address.sin6_family = ltcp->ip_version == SL_IPV6_ADDRESS_LENGTH ? AF_INET6 : AF_INET;
...
ssize_t recvfrom(int socket_id, void *buf, size_t buf_len, int flags, struct sockaddr *addr, socklen_t *addr_len)
{
  ...
  si91x_req_socket_read_t request   = { 0 };
  sl_status_t status                = SL_STATUS_OK;
...
int setsockopt(int socket_id, int option_level, int option_name, const void *option_value, socklen_t option_length)
{
  ...
  switch (option_name) {
    case SO_RCVTIMEO: {
      ...
      timeout = (si91x_timeval_t *)option_value;
    ...
    case TCP_ULP: {
      ...
      if (strncmp(option_value, TLS, option_length) == 0) {
        ...
        si91x_socket->ssl_bitmap = SI91X_SOCKET_FEAT_SSL | SI91X_TLS_V_1_2;
      } else if (strncmp(option_value, TLS_1_1, option_length) == 0) {
        si91x_socket->ssl_bitmap = SI91X_SOCKET_FEAT_SSL | SI91X_TLS_V_1_1;
      } else if (strncmp(option_value, TLS_1_0, option_length) == 0) {
        si91x_socket->ssl_bitmap = SI91X_SOCKET_FEAT_SSL | SI91X_TLS_V_1_0;
      }
#ifdef CHIP_917
      else if (strncmp(option_value, TLS_1_3, option_length) == 0) {
        si91x_socket->ssl_bitmap = SI91X_SOCKET_FEAT_SSL | SI91X_TLS_V_1_3;
      }
#endif
      break;
    }
...
{
  sl_status_t status                = SL_STATUS_OK;
  uint32_t read_select_bitmap       = 0;
  uint32_t write_select_bitmap      = 0;
  int32_t read_count                = 0;
  int32_t write_count               = 0;
  int32_t select_response_wait_time = 0;
  sl_si91x_socket_select_req_t request = { 0 };
  sl_si91x_socket_select_rsp_t *response;
  sl_wifi_buffer_t *buffer;
  sl_si91x_packet_t *packet;
  sl_si91x_wait_period_t wait_time = 0;
  ...
  memset(&request, 0, sizeof(sl_si91x_socket_select_req_t));

  for (uint8_t index = 0; index < nfds; index++) {
    if (readfds != NULL) {
      if ((readfds->__fds_bits[0]) & (1U << index)) {
        read_select_bitmap |= (1U << sockets[index].id);
      }
    }
    if (writefds != NULL) {
      if ((writefds->__fds_bits[0]) & (1U << index)) {
        write_select_bitmap |= (1U << sockets[index].id);
      }
    }
  }

  request.num_fd                    = nfds;
  request.rsi_read_fds.fd_array[0]  = read_select_bitmap;
  request.rsi_write_fds.fd_array[0] = write_select_bitmap;

  if (timeout != NULL && ((timeout->tv_sec != 0) || (timeout->tv_usec != 0))) {
    request.rsi_select_timeval.tv_sec  = timeout->tv_sec;
    request.rsi_select_timeval.tv_usec = timeout->tv_usec;
    select_response_wait_time          = ((request.rsi_select_timeval.tv_sec * 1000)
                                 + (request.rsi_select_timeval.tv_usec / 1000) + SI91X_HOST_WAIT_FOR_SELECT_RSP);
  ...
  status = sl_si91x_driver_send_command(RSI_WLAN_REQ_SELECT_REQUEST,
                                        ...
                                        NULL,
                                        &buffer);
  // TBD: Waiting logic for select data
  BSD_VERIFY_STATUS_AND_RETURN(status, SL_STATUS_OK, SI91X_BSD_UNDEFINED_ERROR);

  /*
  events = si91x_host_wait_for_event(
    NCP_HOST_SOCKET_RESPONSE_EVENT,
    select_response_wait_time != 0 ? SL_SI91X_WAIT_FOR_RESPONSE(select_response_wait_time) : osWaitForever);
  //TODO: Change error handling of response from event to packets

  if ((events & NCP_HOST_SOCKET_RESPONSE_EVENT) != NCP_HOST_SOCKET_RESPONSE_EVENT) {
    SET_ERROR_AND_RETURN(SI91X_BSD_UNDEFINED_ERROR);
  }
  //Note: if firmware delivered the packet after the time limit, the packet is still in the queue
  if (si91x_host_remove_from_queue(SI91X_SOCKET_RESPONSE_QUEUE, &buffer) != SL_STATUS_OK) {
    return -1;
  }
  */
  ...
  if (readfds != NULL) {
    FD_ZERO(readfds);
    memcpy(readfds, &response->rsi_read_fds.fd_array[0], sizeof(fd_set));
    for (uint8_t index = 0; index < 32; index++) {
      if (response->rsi_read_fds.fd_array[0] & (1 << index)) {
        read_count++;
      }
    }
  }
  if (writefds != NULL) {
    FD_ZERO(writefds);
    memcpy(writefds, &response->rsi_write_fds.fd_array[0], sizeof(fd_set));
    for (uint8_t index = 0; index < 32; index++) {
      if (response->rsi_write_fds.fd_array[0] & (1 << index)) {
        write_count++;
      }
    }
  }
