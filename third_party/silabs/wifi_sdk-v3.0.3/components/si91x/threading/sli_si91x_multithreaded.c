...
#else
#define SLNET_EVENT_DISPATCH_HANDLER(data, packet) \
  {                                                \
  }
#endif
...
/*
  ...
  for a particular command of type si91x_command_type_t
*/
typedef struct {
...
#endif
static sl_status_t bus_write_frame(si91x_queue_type_t queue_type,
                                   sl_wifi_buffer_type_t buffer_type,
                                   sl_si91x_command_trace_t *trace,
                                   bool *global_queue_block);
sl_status_t si91x_req_wakeup(void);
#if 0
__WEAK sl_status_t si91x_host_process_data_frame(sl_wifi_interface_t interface, sl_wifi_buffer_t *buffer)
{
...
void si91x_event_handler_thread(void *args)
{
  ...
  uint32_t event                               = 0;
  sl_wifi_buffer_t *network_event_queue_buffer = NULL;
  sl_wifi_buffer_t *wlan_event_queue_buffer    = NULL;
  si91x_queue_packet_t *data                   = NULL;
  sl_si91x_packet_t *packet;
  uint32_t event_mask = 0;
  sl_wifi_event_t wifi_event;

  while (1) {
    event_mask = (NCP_HOST_WLAN_NOTIFICATION_EVENT | NCP_HOST_NETWORK_NOTIFICATION_EVENT);
    event      = si91x_host_wait_for_event(event_mask, osWaitForever);

    if ((event & NCP_HOST_WLAN_NOTIFICATION_EVENT) != 0) {
      while (0 != si91x_host_queue_status(SI91X_WLAN_EVENT_QUEUE)) {
        if (si91x_host_remove_from_queue(SI91X_WLAN_EVENT_QUEUE, &wlan_event_queue_buffer) == SL_STATUS_OK) {
          packet = si91x_host_get_buffer_data(wlan_event_queue_buffer, 0, NULL);

          // Call event handler here
          if ((si91x_event_handler != NULL)
              && (SL_STATUS_OK == convert_si91x_event_to_sl_wifi_event(packet->command, &wifi_event))) {
            si91x_event_handler(wifi_event, wlan_event_queue_buffer);
          }
          si91x_host_free_buffer(wlan_event_queue_buffer, SL_WIFI_RX_FRAME_BUFFER);
        } else {
          // TODO: error handling
        }
      }
    ...
    if ((event & NCP_HOST_NETWORK_NOTIFICATION_EVENT) != 0) {
      while (0 != si91x_host_queue_status(SI91X_NETWORK_EVENT_QUEUE)) {
        if (si91x_host_remove_from_queue(SI91X_NETWORK_EVENT_QUEUE, &network_event_queue_buffer) == SL_STATUS_OK) {
          data   = (si91x_queue_packet_t *)si91x_host_get_buffer_data(network_event_queue_buffer, 0, NULL);
          packet = (sl_si91x_packet_t *)si91x_host_get_buffer_data(data->host_packet, 0, NULL);
          SLNET_EVENT_DISPATCH_HANDLER(data, packet);
          si91x_host_free_buffer(data->host_packet, SL_WIFI_RX_FRAME_BUFFER);
          si91x_host_free_buffer(network_event_queue_buffer, SL_WIFI_RX_FRAME_BUFFER);
        } else {
          // TODO: error handling
        }
      }
...
void si91x_bus_thread(void *args)
{
  ...
  while (1) {
    tx_queues_empty = (si91x_host_queue_status((si91x_queue_type_t)SI91X_COMMON_CMD)
                       | si91x_host_queue_status((si91x_queue_type_t)SI91X_WLAN_CMD)
                       | si91x_host_queue_status((si91x_queue_type_t)SI91X_NETWORK_CMD)
                       | si91x_host_queue_status((si91x_queue_type_t)SI91X_SOCKET_CMD)
                       | si91x_host_queue_status((si91x_queue_type_t)SI91X_BT_CMD));
    ...
    interrupt_status = 0;
    if (si91x_bus_read_interrupt_status(&interrupt_status) != SL_STATUS_OK) {
      continue;
    }

    if (!((interrupt_status & RSI_RX_PKT_PENDING) || (event & NCP_HOST_BUS_RX_EVENT))) {
      if (current_performance_profile != HIGH_PERFORMANCE) {
        si91x_host_clear_sleep_indicator();
      }
    }

    if (((event & NCP_HOST_BUS_RX_EVENT) || (interrupt_status & RSI_RX_PKT_PENDING))
        && (si91x_bus_read_frame(&buffer) == SL_STATUS_OK)) {
      ...
      data = (uint8_t *)si91x_host_get_buffer_data(buffer, 0, &length);
      // Process the frame
      ...
      switch (queue_id) {
          ...
          switch (frame_type) {
            ...
            case RSI_COMMON_RSP_FEATURE_FRAME: {
              ...
              if (((command_trace[SI91X_COMMON_CMD].flags & SI91X_PACKET_RESPONSE_STATUS)
                ...
                status =
                  si91x_host_allocate_buffer(&packet, SL_WIFI_CONTROL_BUFFER, sizeof(si91x_queue_packet_t), 1000);
                node = si91x_host_get_buffer_data(packet, 0, &temp);
                ...
                if (SI91X_PACKET_RESPONSE_PACKET
                  ...
                  si91x_host_free_buffer(buffer, SL_WIFI_CONTROL_BUFFER);
                }
                ...
                si91x_host_add_to_queue(SI91X_COMMON_RESPONSE_QUEUE, packet);
                si91x_host_set_event(NCP_HOST_COMMON_RESPONSE_EVENT);
              } else {
                si91x_host_free_buffer(buffer, SL_WIFI_CONTROL_BUFFER);
              }
              break;
            ...
            case RSI_WLAN_RSP_GAIN_TABLE: {
              ...
              if (((command_trace[SI91X_WLAN_CMD].flags & SI91X_PACKET_RESPONSE_STATUS) == SI91X_PACKET_RESPONSE_STATUS)
                ...
                si91x_host_add_to_queue(SI91X_WLAN_RESPONSE_QUEUE, packet);
                si91x_host_set_event(NCP_HOST_WLAN_RESPONSE_EVENT);
              } else {
                si91x_host_add_to_queue(SI91X_WLAN_EVENT_QUEUE, buffer);
                si91x_host_set_event(NCP_HOST_WLAN_NOTIFICATION_EVENT);
              }
              break;
            }
            ...
            case RSI_WLAN_RSP_REMOTE_TERMINATE: { // This event is received asynchronously when a socket is closed by remote end
              ...
              status = si91x_host_allocate_buffer(&packet, SL_WIFI_CONTROL_BUFFER, sizeof(si91x_queue_packet_t), 1000);
              node   = si91x_host_get_buffer_data(packet, 0, &temp);
              ...
              if (((command_trace[SI91X_NETWORK_CMD].flags & SI91X_PACKET_RESPONSE_STATUS)
                ...
                si91x_host_add_to_queue(SI91X_NETWORK_RESPONSE_QUEUE, packet);
                si91x_host_set_event(NCP_HOST_NETWORK_RESPONSE_EVENT);
              } else {
                si91x_host_add_to_queue(SI91X_NETWORK_EVENT_QUEUE, packet);
                si91x_host_set_event(NCP_HOST_NETWORK_NOTIFICATION_EVENT);
              }
            ...
            case RSI_WLAN_RSP_SOCKET_ACCEPT: {
              ...
              si91x_host_add_to_queue(SI91X_SOCKET_RESPONSE_QUEUE, packet);
              si91x_host_set_event(NCP_HOST_SOCKET_RESPONSE_EVENT);
              break;
            }
            ...
            case RSI_WLAN_RSP_HTTP_CLIENT_POST_DATA: {
              ...
              if (command_trace[SI91X_NETWORK_CMD].frame_type == RSI_WLAN_RSP_HTTP_CLIENT_GET) {
                sl_si91x_packet_t *get_response_packet = si91x_host_get_buffer_data(buffer, 0, NULL);
        ...
        case RSI_WLAN_DATA_Q: {
          ...
          if (frame_type == RSI_RECEIVE_RAW_DATA) {
            ...
            si91x_host_set_event(NCP_HOST_SOCKET_RESPONSE_EVENT);
#else
            si91x_host_process_data_frame(SL_WIFI_CLIENT_INTERFACE, buffer);
            si91x_host_free_buffer(buffer, SL_WIFI_RX_FRAME_BUFFER);
#endif
          }
                                   ...
                                   bool *global_queue_block)
{
  ...
  status = si91x_host_remove_from_queue(queue_type, &buffer);
  VERIFY_STATUS_AND_RETURN(status);

  node            = si91x_host_get_buffer_data(buffer, 0, NULL);
  packet          = si91x_host_get_buffer_data(node->host_packet, 0, NULL);
  ...
  status = si91x_bus_write_frame(packet, packet->data, length);
#ifdef RSI_M4_INTERFACE
  ...
  si91x_host_free_buffer(node->host_packet, buffer_type);
  si91x_host_free_buffer(buffer, buffer_type);
