 ...
 ******************************************************************************/
static sl_status_t sli_fwup(uint16_t type, uint8_t *content, uint16_t length)
{
  sl_status_t status    = SL_STATUS_FAIL;
  si91x_req_fwup_t fwup = { 0 };
...
{
  sl_status_t status = sli_fwup(SL_FWUP_RPS_HEADER, rps_header, SL_RPS_HEADER_SIZE);
  return status;
}

sl_status_t sl_si91x_fwup_load(uint8_t *content, uint16_t length)
{
  sl_status_t status = sli_fwup(SL_FWUP_RPS_CONTENT, content, length);
  return status;
}
...
{
  sl_status_t status                      = SL_STATUS_FAIL;
  si91x_http_client_request_t http_client = { 0 };
  ...
  // Set https feature
  if (flags & SL_SI91X_ENABLE_SSL) {
    https_enable = SL_SI91X_ENABLE_SSL;
  }
  ...
  send_size = sizeof(si91x_http_client_request_t) - SI91X_HTTP_BUFFER_LEN + http_length;
  send_size &= 0xFFF;
