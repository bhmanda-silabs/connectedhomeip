...
static sl_status_t net_wifi_ap_event_handler(sl_wifi_event_t event, sl_wifi_buffer_t *buffer)
{
  ...
  if (user_ap_event_handler) {
    ...
    sl_si91x_packet_t *packet = si91x_host_get_buffer_data(buffer, 0, &data_length);
                                    ...
                                    sl_ip_address_t *sl_ip_address)
{
  ...
  si91x_dns_response_t *dns_response          = { 0 };
  si91x_dns_query_request_t dns_query_request = { 0 };
  ...
  dns_response = (si91x_dns_response_t *)packet->data;
...
sl_status_t sl_si91x_configure_ip_address(sl_net_ip_configuration_t *address, uint8_t virtual_ap_id)
{
  ...
  si91x_req_ipv4_params_t ip_req;
  si91x_req_ipv6_parmas_t ipv6_request;
  sl_si91x_packet_t *packet;
  sl_wifi_buffer_t *buffer;
  ...
  if (SL_IPV4 == address->type) {
    ...
    status = sl_si91x_driver_send_command(RSI_WLAN_REQ_IPCONFV4,
                                          SI91X_NETWORK_CMD_QUEUE,
                                          &ip_req,
                                          sizeof(si91x_req_ipv4_params_t),
    ...
    if (SL_IP_MANAGEMENT_DHCP == address->mode) {
      status = convert_rsi_ipv4_address_to_sl_ip_address((sl_ip_address_t *)&(address->ip.v4.ip_address),
                                                         (si91x_rsp_ipv4_params_t *)packet->data);
      VERIFY_STATUS_AND_RETURN(status);
    }
    ...
    memcpy(&ipv6_request.mode, &address->mode, 2);
    ...
    status = sl_si91x_driver_send_command(RSI_WLAN_REQ_IPCONFV6,
                                          SI91X_NETWORK_CMD_QUEUE,
                                          &ipv6_request,
                                          sizeof(si91x_req_ipv6_parmas_t),
    ...
    uint32_t ip_buffer[13];
    unsigned char link_local_address[46] = { 0 };
    unsigned char global_address[46]     = { 0 };
    unsigned char gateway6[46]           = { 0 };

    memcpy(ip_buffer, packet->data, packet->length);
    sl_inet_ntop6((const unsigned char *)&ip_buffer[1], (char *)&link_local_address, sizeof(link_local_address));
    sl_inet_ntop6((const unsigned char *)&ip_buffer[5], (char *)&global_address, sizeof(global_address));
    sl_inet_ntop6((const unsigned char *)&ip_buffer[9], (char *)&gateway6, sizeof(gateway6));

    SL_DEBUG_LOG("Prefix Length : %d\r\n", ip_buffer[0]);
    SL_DEBUG_LOG("Link Local Address: %s\r\n", link_local_address);
    SL_DEBUG_LOG("Global Address:   %s\r\n", global_address);
    SL_DEBUG_LOG("Gateway Address:  %s\r\n", gateway6);
    si91x_host_free_buffer(buffer, SL_WIFI_RX_FRAME_BUFFER);
