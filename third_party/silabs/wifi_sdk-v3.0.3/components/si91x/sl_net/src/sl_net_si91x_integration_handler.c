 ...
 */
static void handle_mqtt_client_asynch_events(si91x_queue_packet_t *mqtt_asyn_packet)
{
  sl_si91x_packet_t *raw_rx_packet = si91x_host_get_buffer_data(mqtt_asyn_packet->host_packet, 0, NULL);
...
void sl_net_si91x_event_dispatch_handler(si91x_queue_packet_t *data, sl_si91x_packet_t *packet)
{
  sl_status_t status;
  ...
  if (status == SL_STATUS_OK) {
    ...
    sl_si91x_default_handler((sl_si91x_event_t)service_event, data->host_packet);
  }
#ifdef HTTP_CLIENT_FEATURE
  // Check for sl_http_client_event_t
  else if (SL_STATUS_OK == convert_si91x_event_to_sl_http_client_event(&packet->command, &http_event)) {
