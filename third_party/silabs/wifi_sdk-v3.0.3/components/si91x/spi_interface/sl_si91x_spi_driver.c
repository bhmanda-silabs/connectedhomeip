...
sl_status_t si91x_bus_init(void)
{
  uint32_t temp        = htole32(RSI_RS9116_INIT_CMD);
  ...
  do {
    si91x_host_spi_transfer(&temp, rx_buffer, sizeof(uint32_t));
    if (rx_buffer[3] == RSI_SPI_SUCCESS) {
      return SL_STATUS_OK;
    }
  } while (si91x_host_elapsed_time(timestamp) < 10);
...
sl_status_t si91x_bus_set_interrupt_type(uint32_t interrupt_mask)
{
  ...
  status = si91x_bus_read_memory(RSI_INT_MASK_REG_ADDR, sizeof(buffer), buffer);
  ...
  status = si91x_bus_write_memory(RSI_INT_MASK_REG_ADDR, sizeof(buffer), buffer);
  return status;
}
...
{
#ifdef RSI_BIT_32_SUPPORT
  return si91x_bus_write_memory(RSI_INT_CLR_REG_ADDR, 4, &interrupt_clear_value);
#else
  return si91x_bus_write_memory(RSI_INT_CLR_REG_ADDR, 2, (uint8_t *)&interrupt_clear_value);
#endif
}
...
sl_status_t si91x_bus_read_interrupt_status(uint16_t *interrupt_status)
{
  ...
  do {
    ...
    status = si91x_bus_read_register(RSI_SPI_INT_REG_ADDR, 1, interrupt_status);
    if (status != SL_STATUS_BUSY) {
      return status;
    }
  } while (si91x_host_elapsed_time(timestamp) < 1000);
...
sl_status_t si91x_bus_write_memory(uint32_t addr, uint16_t length, const uint8_t *buffer)
{
  ...
  status = si91x_host_spi_transfer(&temp16, NULL, sizeof(uint16_t));
  VERIFY_STATUS(status);

  // Send the 4 address bytes
  temp32 = htole32(addr);
  status = si91x_host_spi_transfer(&temp32, NULL, sizeof(uint32_t));
  VERIFY_STATUS(status);

  // Send the data
  status = si91x_host_spi_transfer(buffer, NULL, length);
  return status;
}
...
sl_status_t si91x_bus_read_memory(uint32_t addr, uint16_t length, uint8_t *buffer)
{
  ...
  // Send C3/C4
  temp16 = htole16(length);
  status = si91x_host_spi_transfer(&temp16, NULL, 2);
  VERIFY_STATUS(status);
  ...
  status = si91x_host_spi_transfer(NULL, buffer, length);
  VERIFY_STATUS(status);
...
static sl_status_t send_c1c2(uint16_t data)
{
  ...
  const uint32_t timestamp = si91x_host_get_timestamp();

  do {
    status = si91x_host_spi_transfer(&data, rx_buffer, 2);
...
sl_status_t si91x_bus_write_register(uint8_t address, uint8_t register_size, uint16_t data)
{
  ...
  status = si91x_host_spi_transfer(&data, NULL, register_size);
  return status;
}
...
sl_status_t si91x_bus_read_register(uint8_t address, uint8_t register_size, uint16_t *output)
{
  ...
  status = si91x_host_spi_transfer(NULL, output, register_size);

  return status;
}

sl_status_t si91x_bus_enable_high_speed(void)
{
  return si91x_bus_write_register(0x08, 2, 0x3);
}
...
static sl_status_t wait_start_token(uint32_t timeout)
{
  ...
  while (temp != RSI_SPI_START_TOKEN) {
    if (si91x_host_elapsed_time(timestamp) > timeout) {
      return SL_STATUS_BUSY;
    }

    status = si91x_host_spi_transfer(NULL, &temp, sizeof(temp));
  }
  return status;
}
...
sl_status_t si91x_bus_read_frame(sl_wifi_buffer_t **buffer)
{
  ...
  // local_buffer[0] = total length
  // local_buffer[1] = dummy length
  ...
  status = si91x_host_allocate_buffer(buffer, SL_WIFI_RX_FRAME_BUFFER, local_buffer[0], 10000);
  VERIFY_STATUS(status);

  data = (uint8_t *)si91x_host_get_buffer_data(*buffer, 0, &temp);
...
static sl_status_t basic_data_transfer(uint16_t c1c2, uint16_t length, void *tx_data, void *rx_data)
{
  ...
  status = si91x_host_spi_transfer(&length, NULL, 2);
  VERIFY_STATUS(status);
  ...
  status = si91x_host_spi_transfer(tx_data, rx_data, length);
  return status;
}
...
static sl_status_t packet_read_with_dummy_data(void *rx_data, uint16_t dummy_length, uint16_t total_length)
{
  ...
  status = si91x_host_spi_transfer(&aligned_len, NULL, 2);
  VERIFY_STATUS(status);
  ...
  status = si91x_host_spi_transfer(NULL, NULL, dummy_length);
  VERIFY_STATUS(status);

  status = si91x_host_spi_transfer(NULL, rx_data, (aligned_len - dummy_length));
  VERIFY_STATUS(status);
...
void rsi_ulp_wakeup_init(void)
{
  ...
  while (1) {
    si91x_host_spi_transfer(txCmd, rxbuff, 2);
    if (rxbuff[1] == RSI_SPI_FAIL) {
      return;
    } else if (rxbuff[1] == 0x00) {
      si91x_host_spi_transfer(&txCmd[2], rxbuff, 2);
