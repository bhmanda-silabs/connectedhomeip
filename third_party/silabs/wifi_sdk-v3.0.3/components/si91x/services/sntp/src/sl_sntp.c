...
sl_status_t si91x_sntp_event_handler(si91x_queue_packet_t *data)
{
  uint16_t status                         = 0;
  sl_sntp_client_response_t response      = { 0 };
  sl_si91x_packet_t *raw_rx_packet        = si91x_host_get_buffer_data(data->host_packet, 0, NULL);
  sl_wifi_buffer_t *sdk_context           = (sl_wifi_buffer_t *)data->sdk_context;
  sl_internal_sntp_client_context_t *node = NULL;
  uint16_t buffer_length                  = 0;

  node = (sl_internal_sntp_client_context_t *)si91x_host_get_buffer_data(sdk_context, 0, &buffer_length);
  ...
  si91x_host_free_buffer(sdk_context, SL_WIFI_CONTROL_BUFFER);
  return SL_STATUS_OK;
}
...
sl_status_t sl_sntp_client_start(sl_sntp_client_config_t *config, uint32_t timeout)
{
  ...
  *((uint16_t *)client_req.sntp_timeout) = config->sntp_timeout;
  ...
  if (timeout > 0) {
    ...
    status =
      si91x_host_allocate_buffer(&sdk_context, SL_WIFI_CONTROL_BUFFER, sizeof(sl_internal_sntp_client_context_t), 1000);
    node                      = si91x_host_get_buffer_data(sdk_context, 0, &buffer_length);
