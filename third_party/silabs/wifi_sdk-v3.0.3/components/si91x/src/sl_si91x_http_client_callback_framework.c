...
sl_status_t sl_si91x_http_client_put_delete(void)
{
  ...
  si91x_http_client_put_request_t *request =
  ...
  // Set packet length
  uint16_t packet_len = sizeof(si91x_http_client_put_request_t) - SI91X_HTTP_CLIENT_PUT_MAX_BUFFER_LENGTH;
...
{
  sl_http_client_callback_entry_t *entry  = get_http_client_callback_entry(event);
  sl_si91x_packet_t *packet               = (sl_si91x_packet_t *)si91x_host_get_buffer_data(buffer, 0, NULL);
  ...
  switch (packet->command) {
    ...
    case RSI_WLAN_RSP_HTTP_CLIENT_PUT: {
      ...
      si91x_http_client_put_pkt_rsp_t *response   = (si91x_http_client_put_pkt_rsp_t *)&packet->data;
      si91x_http_put_pkt_server_rsp_t *server_rsp = (si91x_http_put_pkt_server_rsp_t *)&packet->data;
