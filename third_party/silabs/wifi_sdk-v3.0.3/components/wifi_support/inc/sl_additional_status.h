...
#define SL_STATUS_WIFI_SLEEP_ERROR                         ((sl_status_t)0x0B41) ///< Error sleeping a thread
...
#define SL_STATUS_SI91X_JOIN_AUTHENTICATION_FAILED                              ((sl_status_t)0x1000E) ///< 1. Authentication failure during "Join" 2. Unable to find AP during join which was found during scan.
...
#define SL_STATUS_SI91X_NOT_AP_INTERFACE                                        ((sl_status_t)0x10017) ///< Not a AP interface
...
#define SL_STATUS_SI91X_REJOIN_FAILURE                                          ((sl_status_t)0x10019) ///< 1. Access Point not found 2. Rejoin failure
...
#define SL_STATUS_SI91X_CONCURRENT_MODE_DOWN                                    ((sl_status_t)0x1005C) ///< Concurrent mode, both AP and Client should UP, to enable configuration
...
#define SL_STATUS_SI91X_HIGHER_LISTEN_INTERVAL                                  ((sl_status_t)0x10066) ///< listen interval in power save is greater than the join listen interval
...
#define SL_STATUS_SI91X_PORT_NOT_IN_CLOSE_STATE                                 ((sl_status_t)0x1BB35) ///< Port Not in close state
...
#define SL_STATUS_SI91X_INVALID_COMMAND_OR_OPERATION                            ((sl_status_t)0x1FFF8) ///< 1. Invalid command (e.g. parameters insufficient or invalid in the command). Invalid operation (e.g. power save command with the same mode given twice, accessing wrong socket, creating more than allowed sockets )
...
#define SL_STATUS_SI91X_DNS_CLASS_ERROR_IN_DNS_RESPONSE                         ((sl_status_t)0x1FFBA) ///< DNS Class error in the response to the DNS Resolution query
#define SL_STATUS_SI91X_DNS_COUNT_ERROR_IN_DNS_RESPONSE                         ((sl_status_t)0x1FFB8) ///< DNS count error in the response to the DNS Resolution query
...
#define SL_STATUS_SI91X_MEMORY_LIMIT_EXCEEDED_DURING_AUTO_JOIN                  ((sl_status_t)0x1FF2A) ///< Memory limit exceeded in operating mode during auto join/create
...
#define SL_STATUS_SI91X_FREQUENCY_OFFSET_OUT_OF_LIMITS                          ((sl_status_t)0x100FB) ///< Frequency offset specified goes beyond the upper limit or lower limit and indicates that frequency offset cannot be changed further
