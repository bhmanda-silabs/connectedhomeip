...
sequenceDiagram
    ...
    WiFiLib->>WiFiLib: Change state to inuse
    WiFiLib->>WiFiLib: Fetch the credential using given credential ID

    alt if unable to fetch credential usind given credential ID
        WiFiLib->>AppThread: Return Invalid argumrnt error 
    end
    ...
    Note over WiFiLib: Fill the ssid in join comamnd packet
...
```mermaid
	sequenceDiagram	
		...
		participant app as Appilication
		...
		alt  If device is not in right state
            ...
            par Firmware to Bus thread
                ...
                alt if interrupt status RSI_RX_PKT_PENDING recieved and queue_id == RSI_WLAN_MGMT_Q and frameType == RSI_WLAN_RSP_MULTICAST
	...
	sequenceDiagram	
		...
		alt if version is NULL
            ...
            par Firmware to Bus thread
                Note over bus: Waiting for NCP_HOST_RX_EVENT and rx pending packet
                fw -->> bus: Send NCP_HOST_RX_EVENT event
                alt if interrupt status RSI_RX_PKT_PENDING recieved and queue_id == RSI_WLAN_MGMT_Q and frameType == RSI_WLAN_RSP_FW_VERSION
                    Note over bus: Add buffer to NCP_COMMAND_RESPONSE_QUEUE and set event to NCP_HOST_COMMAND_RESPONSE_EVENT
                end
            end
			...
			par Bus thread to SL WIFI
				...
				bus -->> wifi: Send NCP_HOST_COMMAND_RESPONSE_EVENT event
				alt if command success
					Note over wifi: Read the respone from NCP_COMMAND_RESPONSE_QUEUE and copy it to user's provided memory
					Note over wifi: Free command response buffer
					wifi ->> app: return success
				else 
					wifi ->> app: Return Error
	...
	sequenceDiagram	
		...
		alt  If device is not in right state
            ...
            par Firmware to Bus thread
                Note over bus: Waiting for NCP_HOST_RX_EVENT and rx pending packet
                fw -->> bus: Send NCP_HOST_RX_EVENT event
                alt if interrupt status RSI_RX_PKT_PENDING recieved and queue_id == RSI_WLAN_MGMT_Q and frameType == RSI_WLAN_RSP_MAC_ADDRESS
                    Note over bus: Add buffer to NCP_COMMAND_RESPONSE_QUEUE and set event to NCP_HOST_COMMAND_RESPONSE_EVENT
                end
            end
	...
	sequenceDiagram	
		...
		alt  If device is not in right state
            ...
            par Firmware to Bus thread
                Note over bus: Waiting for NCP_HOST_RX_EVENT and rx pending packet
                fw -->> bus: Send NCP_HOST_RX_EVENT event
                alt if interrupt status RSI_RX_PKT_PENDING recieved and queue_id == RSI_WLAN_MGMT_Q and frameType == RSI_WLAN_RSP_GET_STATS
                    Note over bus: Add buffer to NCP_COMMAND_RESPONSE_QUEUE and set event to NCP_HOST_COMMAND_RESPONSE_EVENT
                end
            end
	...
	sequenceDiagram	
		...
		alt  If device is not in right state
            ...
            par Firmware to Bus thread
                Note over bus: Waiting for NCP_HOST_RX_EVENT and rx pending packet
                fw -->> bus: Send NCP_HOST_RX_EVENT event
                alt if interrupt status RSI_RX_PKT_PENDING recieved and queue_id == RSI_WLAN_MGMT_Q and frameType == RSI_WLAN_RSP_GET_STATUS
                    Note over bus: Add buffer to NCP_COMMAND_RESPONSE_QUEUE and set event to NCP_HOST_COMMAND_RESPONSE_EVENT
                end
            end
	...
	sequenceDiagram	
		...
		alt  If device is not in right state
            ...
            par Firmware to Bus thread
                Note over bus: Waiting for NCP_HOST_RX_EVENT and rx pending packet
                fw -->> bus: Send NCP_HOST_RX_EVENT event
                alt if interrupt status RSI_RX_PKT_PENDING recieved and queue_id == RSI_WLAN_MGMT_Q and frameType == RSI_WLAN_RSP_GET_INTERFACE_STATUS
                    Note over bus: Add buffer to NCP_COMMAND_RESPONSE_QUEUE and set event to NCP_HOST_COMMAND_RESPONSE_EVENT
                end
            end
	...
	sequenceDiagram	
		...
		alt  If device is not in right state
            ...
            par Firmware to Bus thread
                Note over bus: Waiting for NCP_HOST_RX_EVENT and rx pending packet
                fw -->> bus: Send NCP_HOST_RX_EVENT event
                alt if interrupt status RSI_RX_PKT_PENDING recieved and queue_id == RSI_WLAN_MGMT_Q and frameType == RSI_WLAN_RSP_SET_MAC_ADDRESS
                    Note over bus: Add buffer to NCP_COMMAND_RESPONSE_QUEUE and set event to NCP_HOST_COMMAND_RESPONSE_EVENT
                end
            end
...
```mermaid
	sequenceDiagram	
		...
		alt  Check for generic errors 
			...
			Note over wifi: 2. Call to si91x_host_deinit()
			Note over wifi: 3. si91x_host_delay_ms()
			Note over wifi: 4. Call to si91x_bus_deinit()
			Note over wifi: 5. Disable bus interrupt, si91x_host_disable_bus_interrupt
