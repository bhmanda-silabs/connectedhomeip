...
static sl_status_t get_configured_join_request(sl_wifi_interface_t module_interface,
                                               ...
                                               si91x_join_request_t *join_request)
{
...
sl_status_t sl_wifi_set_antenna(sl_wifi_interface_t interface, sl_wifi_antenna_t antenna)
{
  ...
  sl_status_t status;
  rsi_antenna_select_t rsi_antenna = { 0 };
...
sl_status_t sl_wifi_wait_for_scan_results(sl_wifi_scan_result_t **scan_results, uint32_t max_scan_result_count)
{
  ...
  status = si91x_host_remove_from_queue(SI91X_WLAN_RESPONSE_QUEUE, &buffer);
  VERIFY_STATUS_AND_RETURN(status);
                               ...
                               const sl_wifi_scan_configuration_t *configuration)
{
  ...
  if (SL_WIFI_SCAN_TYPE_ADV_SCAN != configuration->type) {
    ...
    rsi_req_bg_scan_t scan_request = { 0 };
...
{
  sl_status_t status;
  rsi_req_psk_t psk_request;
  rsi_req_eap_config_t eap_req = { 0 };
  si91x_join_request_t join_request;
  sl_wifi_buffer_t *buffer;
  ...
  if ((SL_WIFI_WPA_ENTERPRISE == ap->security) || (SL_WIFI_WPA2_ENTERPRISE == ap->security)) {
    ...
    status = si91x_host_get_credentials(ap->credential_id, SL_WIFI_CRED_EAP, &cred);
    VERIFY_STATUS_AND_RETURN(status);
    ...
    status = si91x_host_get_credentials(ap->credential_id, SL_WIFI_CRED_PSK, &cred);
    VERIFY_STATUS_AND_RETURN(status);
...
sl_status_t sl_wifi_get_signal_strength(sl_wifi_interface_t interface, int32_t *rssi)
{
  ...
  sl_si91x_packet_t *packet = si91x_host_get_buffer_data(buffer, 0, NULL);
  *rssi                     = -(packet->data[0]);
  si91x_host_free_buffer(buffer, SL_WIFI_RX_FRAME_BUFFER);
  return SL_STATUS_OK;
}
...
sl_status_t sl_wifi_get_channel(sl_wifi_interface_t interface, sl_wifi_channel_t *channel_info)
{
  ...
  switch (interface) {
    case SL_WIFI_CLIENT_2_4GHZ_INTERFACE: {
      channel_info->channel = ((si91x_network_params_response_t *)packet->data)->channel_number;
      channel_info->band    = SL_WIFI_BAND_2_4GHZ;
      break;
    }
    case SL_WIFI_AP_2_4GHZ_INTERFACE: {
      memcpy(&channel_info->channel, ((si91x_client_info_response *)packet->data)->channel_number, 2);
      channel_info->band = SL_WIFI_BAND_2_4GHZ;
      break;
    }
    default:
...
sl_status_t sl_wifi_start_ap(sl_wifi_interface_t interface, const sl_wifi_ap_configuration_t *configuration)
{
  ...
  si91x_ap_config_request request   = { 0 };
  si91x_join_request_t join_request = { 0 };
  sl_wifi_credential_t cred         = { 0 };
  ...
  status = si91x_host_get_credentials(configuration->credential_id, SL_WIFI_CRED_PSK, &cred);
  VERIFY_STATUS_AND_RETURN(status);
  ...
  join_response = si91x_host_get_buffer_data(rx_buffer, 0, NULL);

  if (join_response->data[0] != 'G') {
    si91x_host_free_buffer(rx_buffer, SL_WIFI_RX_FRAME_BUFFER);
    return SL_STATUS_NOT_AVAILABLE;
  }
...
sl_status_t sl_wifi_get_pairwise_master_key(sl_wifi_interface_t interface,
                                            const char *psk,
                                            uint8_t psk_length,
                                            uint8_t (*pmk)[SL_WIFI_MAX_PMK_LENGTH])
{
  UNUSED_PARAMETER(interface);
  UNUSED_PARAMETER(psk);
  UNUSED_PARAMETER(psk_length);
  sl_status_t status;
  sl_wifi_buffer_t *buffer;
  rsi_req_psk_t request = { .type = 0, .ap_ssid = { 0 } };
  ...
  sl_si91x_packet_t *packet = si91x_host_get_buffer_data(buffer, 0, NULL);
  if (packet->length != SL_WIFI_MAX_PMK_LENGTH) {
    return SL_STATUS_FAIL;
  }
  memcpy(pmk, packet->data, SL_WIFI_MAX_PMK_LENGTH);
  si91x_host_free_buffer(buffer, SL_WIFI_RX_FRAME_BUFFER);
  return SL_STATUS_OK;
}
                                         ...
                                         sl_wifi_deauth_reason_t reason)
{
  ...
  si91x_disassociation_request_t disconnect_request = { 0 };
...
sl_status_t sl_wifi_get_ap_client_info(sl_wifi_interface_t interface, sl_wifi_client_info_response *client_info)
{
  ...
  sl_wifi_buffer_t *buffer;
  sl_si91x_packet_t *packet;
  si91x_client_info_response si91x_client_info_response = { 0 };
  ...
  memcpy(&si91x_client_info_response, packet->data, sizeof(si91x_client_info_response));
  convert_si91x_wifi_client_info(client_info, &si91x_client_info_response);
...
sl_status_t sl_wifi_start_statistic_report(sl_wifi_interface_t interface, sl_wifi_channel_t channel)
{
  ...
  sl_status_t status            = SL_STATUS_OK;
  si91x_req_rx_stats_t rx_stats = { 0 };
...
sl_status_t sl_wifi_generate_wps_pin(sl_wifi_wps_pin_t *wps_pin)
{
  ...
  sl_si91x_packet_t *packet                     = NULL;
  si91x_wps_method_request_t wps_method_request = { 0 };
  ...
  status = sl_si91x_driver_send_command(RSI_WLAN_REQ_WPS_METHOD,
                                        SI91X_WLAN_CMD_QUEUE,
                                        &wps_method_request,
                                        sizeof(si91x_wps_method_request_t),
                                        SL_SI91X_WAIT_FOR_COMMAND_RESPONSE,
                                        NULL,
                                        &buffer);
  VERIFY_STATUS_AND_RETURN(status);
                              ...
                              const sl_wifi_wps_pin_t *optional_wps_pin)
{
  ...
  sl_status_t status;
  si91x_join_request_t wps_button_press_request = { 0 };
...
{
  si91x_req_roam_params_t roam_param_request = { 0 };
  ...
  sl_status_t status = sl_si91x_driver_send_command(RSI_WLAN_REQ_ROAM_PARAMS,
                                                    SI91X_WLAN_CMD_QUEUE,
                                                    &roam_param_request,
                                                    sizeof(si91x_req_roam_params_t),
                                                    SL_SI91X_WAIT_FOR_COMMAND_SUCCESS,
                                                    NULL,
                                                    NULL);
...
{
  sl_status_t status                                = SL_STATUS_OK;
  si91x_request_wlan_filter_bcast_t sl_filter_bcast = { 0 };
  ...
  status = sl_si91x_driver_send_command(RSI_WLAN_REQ_FILTER_BCAST_PACKETS,
                                        SI91X_WLAN_CMD_QUEUE,
                                        &sl_filter_bcast,
                                        sizeof(si91x_request_wlan_filter_bcast_t),
...
{
  sl_status_t status                          = SL_STATUS_OK;
  si91x_gain_table_info_t *sl_gain_table_info = malloc(sizeof(si91x_gain_table_info_t) + payload_length);
  ...
  status = sl_si91x_driver_send_command(RSI_WLAN_REQ_GAIN_TABLE,
                                        SI91X_WLAN_CMD_QUEUE,
                                        sl_gain_table_info,
                                        sizeof(si91x_gain_table_info_t) + (sl_gain_table_info->size),
                                        SL_SI91X_WAIT_FOR(30100),
                                        NULL,
                                        NULL);
...
sl_status_t sl_wifi_set_11ax_config(uint8_t guard_interval)
{
  ...
  return SL_STATUS_NOT_SUPPORTED;
#endif
  si91x_11ax_config_params_t config_11ax_params = { 0 };
