...
sl_status_t sl_http_client_init(const sl_http_client_configuration_t *client_configuration, sl_http_client_t *client)
{
  ...
  if ((client_configuration->certificate_index < SL_HTTPS_CLIENT_DEFAULT_CERTIFICATE_INDEX)
      || (client_configuration->certificate_index > SL_HTTPS_CLIENT_CERTIFICATE_INDEX_2)) {
                                            ...
                                            const sl_http_client_request_t *request)
{
  ...
  si91x_http_client_request_t *http_client_request =
    (si91x_http_client_request_t *)malloc(sizeof(si91x_http_client_request_t));
  VERIFY_MALLOC_AND_RETURN(http_client_request);

  memset(http_client_request, 0, sizeof(si91x_http_client_request_t));
  ...
  if (client_internal->configuration.https_enable) {
    // Enable SSL/TLS
    http_client_request->https_enable |= SL_SI91X_ENABLE_SSL;
  ...
  packet_length = sizeof(si91x_http_client_request_t) - SI91X_HTTP_BUFFER_LEN + http_buffer_offset;
...
sl_status_t sl_http_client_send_request(const sl_http_client_t *client, const sl_http_client_request_t *request)
{
  ...
  switch (request->http_method_type) {
    ...
    case SL_HTTP_PUT: {
      ...
      // Allocate memory for request structure
      si91x_http_client_put_request_t *http_put_request =
        (si91x_http_client_put_request_t *)malloc(sizeof(si91x_http_client_put_request_t));
      VERIFY_MALLOC_AND_RETURN(http_put_request);

      memset(http_put_request, 0, sizeof(si91x_http_client_put_request_t));

      si91x_http_client_put_start_t *http_put_start = &http_put_request->http_client_put_struct.http_client_put_start;
      ...
      packet_length = sizeof(si91x_http_client_put_request_t) - SI91X_HTTP_CLIENT_PUT_MAX_BUFFER_LENGTH;
      ...
      if (http_client_handle.configuration.https_enable) {
        http_put_start->https_enable = SL_SI91X_ENABLE_SSL;
      ...
      // Fill data length
      packet_length =
        sizeof(si91x_http_client_put_request_t) - SI91X_HTTP_CLIENT_PUT_MAX_BUFFER_LENGTH + http_buffer_offset;
                                              ...
                                              bool flush_now)
{
  ...
  switch (http_client_handle.request.http_method_type) {
    case SL_HTTP_POST: {
      // Allocate memory for request structure
      si91x_http_client_post_data_request_t *http_post_data =
        (si91x_http_client_post_data_request_t *)malloc(sizeof(si91x_http_client_post_data_request_t));
      ...
      // Fill total packet length
      packet_length =
        sizeof(si91x_http_client_post_data_request_t) - SI91X_HTTP_CLIENT_POST_MAX_BUFFER_LENGTH + data_length;
    ...
    case SL_HTTP_PUT: {
      // Allocate memory for request structure
      si91x_http_client_put_request_t *http_put_pkt_request =
        (si91x_http_client_put_request_t *)malloc(sizeof(si91x_http_client_put_request_t));
      VERIFY_MALLOC_AND_RETURN(http_put_pkt_request);

      si91x_http_client_put_data_request_t *http_put_data =
      ...
      // Fill total packet length
      packet_length = sizeof(si91x_http_client_put_request_t) - SI91X_HTTP_CLIENT_PUT_MAX_BUFFER_LENGTH + data_length;
