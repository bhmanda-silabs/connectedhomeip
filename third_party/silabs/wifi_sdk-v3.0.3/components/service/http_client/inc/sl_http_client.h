 ...
 * @note 917 does not support SL_TLS_V_1_3 in current release
 */
typedef enum { SL_TLS_V_1_0 = 0, SL_TLS_V_1_1 = 1, SL_TLS_V_1_2 = 2, SL_TLS_V_1_3 = 3 } sl_http_client_tls_version_t;

/// HTTP versions
typedef enum { SL_HTTP_V_1_0 = 0, SL_HTTP_V_1_1 = 1 } sl_http_client_version_t;
...
/***************************************************************************/ /**
 * @brief
 *    Callback invoked when a http response is received
 * @param[out] client
 *    HTTP clients handle
 * @param[out] event
 *    HTTP event which is occurred
 * @param[out] data
 *    HTTP response data
...
/// HTTP client extended header node structure
...
typedef struct {
  ...
  uint8_t *body; ///< HTTP body to be sent to server, SET this to NULL will process the request in chunked encoding
  uint32_t
...
/***************************************************************************/ /**
 * @brief
 *   Denitializes a HTTP client and releases resources used by HTTP client.
 * @pre 
 ...
 *   If there are any extended headers that have not been deleted, this function will take care of deleting them.
 ******************************************************************************/
sl_status_t sl_http_client_deinit(sl_http_client_t *client);

/***************************************************************************/ /**
 * @brief
 *   Initializes callback function for HTTP method requested.
 * @pre 
 *   @ref sl_http_client_init should be called before this API.
 * @param[in] request
 ...
 *   sl_status_t. See https://docs.silabs.com/gecko-platform/4.1/common/api/group-status for details.
 * @note
 *   If request does not contain any extended header, this API will allocate memory for it.
 ******************************************************************************/
...
/***************************************************************************/ /**
 * @brief
 *   Deletes all the headers from extended header of an HTTP client request
 * @pre 
 *   @ref sl_http_client_add_header should be called before this API.
 * @param[in] request
 *   HTTP client request configuration
...
/***************************************************************************/ /**
 * @brief
 *   Sends HTTP get/post/head/put/delete based on sl_http_client_method_type_t selected
 * @pre
 ...
 *   sl_status_t. See https://docs.silabs.com/gecko-platform/4.1/common/api/group-status for details.
 * @note
 *   1. HTTP head and delete not supported in the current release
 *   2. body_length header in request is added by default by TA
