...
#define SCB_t ((SCB_Type_t *)SCB_BASE) /*!< SCB configuration struct           */
...
void RSI_Default_Reset_Handler(void)
{
  ...
  memcpy(RAM_VECTOR, (uint32_t *)SCB_t->VTOR, sizeof(g_pfnVectors));
  //assing the ram vector address to VTOR register
  SCB_t->VTOR = (uint32_t)RAM_VECTOR;
#endif
 ...
 */
#include "sl_device.h"
volatile uint32_t return_from_default;
static void Default_Handler(void)
{
  return_from_default = 0;
  /* Go into an infinite loop. */
  while (!return_from_default) {
  }
}
