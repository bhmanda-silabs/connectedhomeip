    ...
    case ARM_SPI_CONTROL_SS: // Control Slave Select; arg = 0:inactive, 1:active
      ...
      if (arg == ARM_SPI_SS_INACTIVE) {
        if (val == ARM_SPI_SS_MASTER_HW_OUTPUT) {
          ...
          if (spi->io.cs0 != NULL) {
            RSI_EGPIO_SetPin(EGPIO, spi->io.cs0->port, spi->io.cs0->pin, 1);
          }
          if (spi->io.cs1 != NULL) {
            RSI_EGPIO_SetPin(EGPIO, spi->io.cs1->port, spi->io.cs1->pin, 1);
          }
          if (spi->io.cs2 != NULL) {
            RSI_EGPIO_SetPin(EGPIO, spi->io.cs2->port, spi->io.cs2->pin, 1);
          }
          if (spi->io.cs3 != NULL) {
            RSI_EGPIO_SetPin(EGPIO, spi->io.cs3->port, spi->io.cs3->pin, 1);
          }
        }
        ...
        if (val == ARM_SPI_SS_MASTER_HW_OUTPUT) {
          ...
          if (spi->io.cs0 != NULL) {
            RSI_EGPIO_SetPin(EGPIO, spi->io.cs0->port, spi->io.cs0->pin, 0);
          }
          if (spi->io.cs1 != NULL) {
            RSI_EGPIO_SetPin(EGPIO, spi->io.cs1->port, spi->io.cs1->pin, 0);
          }
          if (spi->io.cs2 != NULL) {
            RSI_EGPIO_SetPin(EGPIO, spi->io.cs2->port, spi->io.cs2->pin, 0);
          }
          if (spi->io.cs3 != NULL) {
            RSI_EGPIO_SetPin(EGPIO, spi->io.cs3->port, spi->io.cs3->pin, 0);
          }
        }
      }
      return ARM_DRIVER_OK;
    ...
    switch (control & ARM_SPI_SS_MASTER_MODE_Msk) {
      ...
      case ARM_SPI_SS_MASTER_SW: // SPI Slave Select when Master: Software controlled
        ...
        if (spi->io.cs0->pin != NULL) {
          ...
          RSI_EGPIO_SetPinMux(EGPIO, spi->io.cs0->port, spi->io.cs0->pin, 0);
          if (spi->io.cs0->pad_sel != 0) {
            RSI_EGPIO_PadSelectionEnable(spi->io.cs0->pad_sel);
          }
          RSI_EGPIO_SetDir(EGPIO, spi->io.cs0->port, spi->io.cs0->pin, 0);
          RSI_EGPIO_SetPin(EGPIO, spi->io.cs0->port, spi->io.cs0->pin, 1);
          spi->info->mode |= ARM_SPI_SS_MASTER_SW;
        }
        if (spi->io.cs1->pin != NULL) {
          if (spi->io.cs1->pin > 64) {
            RSI_EGPIO_SetPinMux(EGPIO1, 0, (spi->io.cs1->pin - 64), EGPIO_PIN_MUX_MODE6);
          }
          RSI_EGPIO_SetPinMux(EGPIO, spi->io.cs1->port, spi->io.cs1->pin, 0);
          if (spi->io.cs1->pad_sel != 0) {
            RSI_EGPIO_PadSelectionEnable(spi->io.cs1->pad_sel);
          }
          RSI_EGPIO_SetDir(EGPIO, spi->io.cs1->port, spi->io.cs1->pin, 0);
          RSI_EGPIO_SetPin(EGPIO, spi->io.cs1->port, spi->io.cs1->pin, 1);
          spi->info->mode |= ARM_SPI_SS_MASTER_SW;
        }
        if (spi->io.cs2->pin != NULL) {
          if (spi->io.cs2->pin > 64) {
            RSI_EGPIO_SetPinMux(EGPIO1, 0, (spi->io.cs2->pin - 64), EGPIO_PIN_MUX_MODE6);
          }
          RSI_EGPIO_SetPinMux(EGPIO, spi->io.cs2->port, spi->io.cs2->pin, 0);
          if (spi->io.cs2->pad_sel != 0) {
            RSI_EGPIO_PadSelectionEnable(spi->io.cs2->pad_sel);
          }
          RSI_EGPIO_SetDir(EGPIO, spi->io.cs2->port, spi->io.cs2->pin, 0);
          RSI_EGPIO_SetPin(EGPIO, spi->io.cs2->port, spi->io.cs2->pin, 1);
          spi->info->mode |= ARM_SPI_SS_MASTER_SW;
        }
        if (spi->io.cs3->pin != NULL) {
          if (spi->io.cs3->pin > 64) {
            RSI_EGPIO_SetPinMux(EGPIO1, 0, (spi->io.cs3->pin - 64), EGPIO_PIN_MUX_MODE6);
          }
          RSI_EGPIO_SetPinMux(EGPIO, spi->io.cs3->port, spi->io.cs3->pin, 0);
          if (spi->io.cs3->pad_sel != 0) {
            RSI_EGPIO_PadSelectionEnable(spi->io.cs3->pad_sel);
          }
          RSI_EGPIO_SetDir(EGPIO, spi->io.cs3->port, spi->io.cs3->pin, 0);
          RSI_EGPIO_SetPin(EGPIO, spi->io.cs3->port, spi->io.cs3->pin, 1);
          spi->info->mode |= ARM_SPI_SS_MASTER_SW;
        } else {
          return ARM_SPI_ERROR_SS_MODE;
...
// ROM API Structure enable in linux
#if 0
const ROM_SSI_API_T ssi_api = {
  &SPI_Initialize,   &SPI_Uninitialize, &SPI_PowerControl, &SPI_Send,          &SPI_Receive,       &SPI_Transfer,
  &SPI_GetDataCount, &SPI_Control,      &SPI_GetStatus,    &SPI_UDMA_Tx_Event, &SPI_UDMA_Rx_Event, &SPI_IRQHandler,
};
#endif
#endif
