...
{
#if RSI_M4_INTERFACE
  si91x_boot_configuration_t boot_config = { 0 };
  get_saved_boot_configuration(boot_config);

  if (boot_config.custom_feature_bit_map & SL_SI91X_FEAT_CUSTOM_FEAT_EXTENTION_VALID) {
    uint32_t switch_sel = 0;
    switch_sel          = (boot_config.ext_custom_feature_bit_map & (BIT(29) | (BIT(30))));
                            ...
                            uint32_t mode)
{
  ...
  if ((P2P_STATUS_REG & TA_wakeup_M4) || (P2P_STATUS_REG & M4_wakeup_TA)
      || ((si91x_host_queue_status((si91x_queue_type_t)SI91X_COMMON_CMD)
...
#ifdef CHIP_917
  //!Upon wakeup program wireless gpio frontend switch controls
