...
static int32_t wfx_rsi_init(void)
{
  ...
  status = sl_wifi_get_mac_address(interface, &wfx_rsi.sta_mac.octet[0]);
  VERIFY_STATUS_AND_RETURN(status);

  WFX_RSI_LOG("%x:%x:%x:%x:%x:%x",
...
//wifi-sdk
       ...
       WFX_RSI_LOG("%s: in joinnnnnnnnnnnnnnnnnnnnnnnn", __func__);
    int32_t status;
    ...
    else
    {
        ...
        WFX_RSI_LOG("sl_wifi_set_join_callback  afetr");
...
//        }
         ...
         */
            ...
            WFX_RSI_LOG("sl_net_set_credential  before");
           ...
           ap.credential_id = id;
            WFX_RSI_LOG("sl_net_set_credential  after %%%%%%%%%%");
//              while (is_wifi_disconnection_event || wfx_rsi.join_retries <= WFX_RSI_CONFIG_MAX_JOIN)
//        {
           if ((status =  sl_wifi_connect(interface, &ap, timeout_ms)) == SL_STATUS_IN_PROGRESS)
           {
         ...
         //     WFX_RSI_LOG("sl_wifi_connect  before");
       //    wfx_rsi.dev_state &= ~WFX_RSI_ST_STA_CONNECTING;
     //      wfx_retry_interval_handler(is_wifi_disconnection_event, wfx_rsi.join_retries);
...
int32_t wfx_rsi_send_data(void * p, uint16_t len)
{
 ...
 // packet->command = RSI_SEND_RAW_DATA;
  ...
  {
      WFX_RSI_LOG("*ERR*EN-RSI:Send fail");
      return ERR_IF;
  }
  return status;
}
